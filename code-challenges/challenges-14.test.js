...
const screenForNames = (arr) => {
  // Solution code here...
  return arr.filter(result => /^((Mr||Mrs||Ms||Dr).\s)[A-Z]/.test(result));
}
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  const newArray = arr.reduce((name, mass) => {
...
Instead of trying to write crazy for loops to automate checking the rows, columns and diagonals consider writing one helper function that accepts three coordinate pairs and checks the values of the array at those locations. For instance checking(row1, col1, row2, col2, row3, col3).
...
const detectTicTacToeWin = (board) => {
  ...
  const checking = (row1, col1, row2, col2, row3, col3) => {
    ...
    return (board[row1][col2] !== '') && (board[row1][col1] === board[row2][col2]) && (board[row1][col1] === board[row3][col3]);
  };
  if (checking(0,0, 0,1, 0,2)){
    return true;
  }
  if (checking(1,0, 1,1, 1,2)){
    return true;
  }
  if (checking(2,0, 2,1, 2,2)){
    return true;
  }
  if (checking(0,0, 1,0, 2,0)){
    return true;
  }
  if (checking(0,1, 1,1, 2,1)){
    return true;
  }
  if (checking(0,2, 1,2, 2,2)){
    return true;
  }
  if (checking(0,0, 1,1, 2,2)){
    return true;
  }
  if (checking(0,2, 1,1, 2,0)){
    return true;
  }
  else {
    return false;
